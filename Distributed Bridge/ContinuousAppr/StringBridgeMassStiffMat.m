...
MassMatTrapez = zeros(modesNumber);
MassMatAnalytic = zeros(modesNumber);
StiffMatAnalytic = zeros(modesNumber);
...
%         MassMatTrapez(n,m) = rA * psim.'*psin*h;
        MassMatAnalytic(n,m) = rA*ComputeMassMat(m,n, 0,L, L, lambdaPm,lambdaMm, lambdaPn,lambdaMn);
        StiffMatAnalytic(n,m) = ComputeStiffMat(m, n, 0, L, L, lambdaPm, lambdaMm, lambdaPn, lambdaMn, T0, EI);
    end
end 
invMassMatAnalytic = MassMatAnalytic^-1;

[V,D] = eig(StiffMatAnalytic,MassMatAnalytic) ;
%[Vm,Dm] = eig(MassMat) ;

omm = sort(sqrt(diag(D))) ;

err_percent = (omm(1:modesNumber)-eigenFreqs(1:modesNumber))./eigenFreqs(1:modesNumber)*100
...
function f = ComputeMassMat(m, n, x1, x2, L, lambdaPm, lambdaMm, lambdaPn, lambdaMn)
    constm = lambdaMm^2*sin(lambdaMm*L)/lambdaPm^2/sinh(lambdaPm*L);
    constn = lambdaMn^2*sin(lambdaMn*L)/lambdaPn^2/sinh(lambdaPn*L);
    if m ~= n
        f =  (lambdaMn*cos(lambdaMn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm*cos(lambdaMm*x2)*sin(lambdaMn*x2))/(lambdaMm^2 - lambdaMn^2) - (constm*lambdaMn*cos(lambdaMn*x2)*sinh(lambdaPm*x2))/(lambdaMn^2 + lambdaPm^2) - (constn*lambdaMm*cos(lambdaMm*x2)*sinh(lambdaPn*x2))/(lambdaMm^2 + lambdaPn^2) + (constm*lambdaPm*cosh(lambdaPm*x2)*sin(lambdaMn*x2))/(lambdaMn^2 + lambdaPm^2) + (constn*lambdaPn*cosh(lambdaPn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 + lambdaPn^2) + (constm*constn*lambdaPm*cosh(lambdaPm*x2)*sinh(lambdaPn*x2))/(lambdaPm^2 - lambdaPn^2) - (constm*constn*lambdaPn*cosh(lambdaPn*x2)*sinh(lambdaPm*x2))/(lambdaPm^2 - lambdaPn^2) - ...
            (lambdaMn*cos(lambdaMn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm*cos(lambdaMm*x1)*sin(lambdaMn*x1))/(lambdaMm^2 - lambdaMn^2) - (constm*lambdaMn*cos(lambdaMn*x1)*sinh(lambdaPm*x1))/(lambdaMn^2 + lambdaPm^2) - (constn*lambdaMm*cos(lambdaMm*x1)*sinh(lambdaPn*x1))/(lambdaMm^2 + lambdaPn^2) + (constm*lambdaPm*cosh(lambdaPm*x1)*sin(lambdaMn*x1))/(lambdaMn^2 + lambdaPm^2) + (constn*lambdaPn*cosh(lambdaPn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 + lambdaPn^2) + (constm*constn*lambdaPm*cosh(lambdaPm*x1)*sinh(lambdaPn*x1))/(lambdaPm^2 - lambdaPn^2) - (constm*constn*lambdaPn*cosh(lambdaPn*x1)*sinh(lambdaPm*x1))/(lambdaPm^2 - lambdaPn^2);
    else
        f = x2/2 - (constm^2*x2)/2 - (cos(lambdaMm*x2)*sin(lambdaMm*x2))/(2*lambdaMm) + (constm^2*cosh(lambdaPm*x2)*sinh(lambdaPm*x2))/(2*lambdaPm) - (2*constm*lambdaMm*cos(lambdaMm*x2)*sinh(lambdaPm*x2))/(lambdaMm^2 + lambdaPm^2) + (2*constm*lambdaPm*cosh(lambdaPm*x2)*sin(lambdaMm*x2))/(lambdaMm^2 + lambdaPm^2) - ...
            x1/2 - (constm^2*x1)/2 - (cos(lambdaMm*x1)*sin(lambdaMm*x1))/(2*lambdaMm) + (constm^2*cosh(lambdaPm*x1)*sinh(lambdaPm*x1))/(2*lambdaPm) - (2*constm*lambdaMm*cos(lambdaMm*x1)*sinh(lambdaPm*x1))/(lambdaMm^2 + lambdaPm^2) + (2*constm*lambdaPm*cosh(lambdaPm*x1)*sin(lambdaMm*x1))/(lambdaMm^2 + lambdaPm^2);
    end
end

function f = ComputeStiffMat(m, n, x1, x2, L, lambdaPm, lambdaMm, lambdaPn, lambdaMn, T0, EI)
    constm = lambdaMm^2*sin(lambdaMm*L)/lambdaPm^2/sinh(lambdaPm*L);
    constn = lambdaMn^2*sin(lambdaMn*L)/lambdaPn^2/sinh(lambdaPn*L);
    
    if m ~= n
        int1 = (lambdaMm^2*lambdaMn*cos(lambdaMn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm*lambdaMn^2*cos(lambdaMm*x2)*sin(lambdaMn*x2))/(lambdaMm^2 - lambdaMn^2) + (constm*lambdaMn*lambdaPm^2*cos(lambdaMn*x2)*sinh(lambdaPm*x2))/(lambdaMn^2 + lambdaPm^2) + (constm*lambdaMn^2*lambdaPm*cosh(lambdaPm*x2)*sin(lambdaMn*x2))/(lambdaMn^2 + lambdaPm^2) + (constn*lambdaMm*lambdaPn^2*cos(lambdaMm*x2)*sinh(lambdaPn*x2))/(lambdaMm^2 + lambdaPn^2) + (constn*lambdaMm^2*lambdaPn*cosh(lambdaPn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 + lambdaPn^2) - (constm*constn*lambdaPm*lambdaPn^2*cosh(lambdaPm*x2)*sinh(lambdaPn*x2))/(lambdaPm^2 - lambdaPn^2) + (constm*constn*lambdaPm^2*lambdaPn*cosh(lambdaPn*x2)*sinh(lambdaPm*x2))/(lambdaPm^2 - lambdaPn^2) - ...
            (lambdaMm^2*lambdaMn*cos(lambdaMn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm*lambdaMn^2*cos(lambdaMm*x1)*sin(lambdaMn*x1))/(lambdaMm^2 - lambdaMn^2) + (constm*lambdaMn*lambdaPm^2*cos(lambdaMn*x1)*sinh(lambdaPm*x1))/(lambdaMn^2 + lambdaPm^2) + (constm*lambdaMn^2*lambdaPm*cosh(lambdaPm*x1)*sin(lambdaMn*x1))/(lambdaMn^2 + lambdaPm^2) + (constn*lambdaMm*lambdaPn^2*cos(lambdaMm*x1)*sinh(lambdaPn*x1))/(lambdaMm^2 + lambdaPn^2) + (constn*lambdaMm^2*lambdaPn*cosh(lambdaPn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 + lambdaPn^2) - (constm*constn*lambdaPm*lambdaPn^2*cosh(lambdaPm*x1)*sinh(lambdaPn*x1))/(lambdaPm^2 - lambdaPn^2) + (constm*constn*lambdaPm^2*lambdaPn*cosh(lambdaPn*x1)*sinh(lambdaPm*x1))/(lambdaPm^2 - lambdaPn^2);
 
        int2 = (lambdaMm^4*lambdaMn^3*cos(lambdaMn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm^3*lambdaMn^4*cos(lambdaMm*x2)*sin(lambdaMn*x2))/(lambdaMm^2 - lambdaMn^2) - (constm*lambdaMn^3*lambdaPm^4*cos(lambdaMn*x2)*sinh(lambdaPm*x2))/(lambdaMn^2 + lambdaPm^2) - (constm*lambdaMn^4*lambdaPm^3*cosh(lambdaPm*x2)*sin(lambdaMn*x2))/(lambdaMn^2 + lambdaPm^2) - (constn*lambdaMm^3*lambdaPn^4*cos(lambdaMm*x2)*sinh(lambdaPn*x2))/(lambdaMm^2 + lambdaPn^2) - (constn*lambdaMm^4*lambdaPn^3*cosh(lambdaPn*x2)*sin(lambdaMm*x2))/(lambdaMm^2 + lambdaPn^2) - (constm*constn*lambdaPm^3*lambdaPn^4*cosh(lambdaPm*x2)*sinh(lambdaPn*x2))/(lambdaPm^2 - lambdaPn^2) + (constm*constn*lambdaPm^4*lambdaPn^3*cosh(lambdaPn*x2)*sinh(lambdaPm*x2))/(lambdaPm^2 - lambdaPn^2) - ...
            (lambdaMm^4*lambdaMn^3*cos(lambdaMn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 - lambdaMn^2) - (lambdaMm^3*lambdaMn^4*cos(lambdaMm*x1)*sin(lambdaMn*x1))/(lambdaMm^2 - lambdaMn^2) - (constm*lambdaMn^3*lambdaPm^4*cos(lambdaMn*x1)*sinh(lambdaPm*x1))/(lambdaMn^2 + lambdaPm^2) - (constm*lambdaMn^4*lambdaPm^3*cosh(lambdaPm*x1)*sin(lambdaMn*x1))/(lambdaMn^2 + lambdaPm^2) - (constn*lambdaMm^3*lambdaPn^4*cos(lambdaMm*x1)*sinh(lambdaPn*x1))/(lambdaMm^2 + lambdaPn^2) - (constn*lambdaMm^4*lambdaPn^3*cosh(lambdaPn*x1)*sin(lambdaMm*x1))/(lambdaMm^2 + lambdaPn^2) - (constm*constn*lambdaPm^3*lambdaPn^4*cosh(lambdaPm*x1)*sinh(lambdaPn*x1))/(lambdaPm^2 - lambdaPn^2) + (constm*constn*lambdaPm^4*lambdaPn^3*cosh(lambdaPn*x1)*sinh(lambdaPm*x1))/(lambdaPm^2 - lambdaPn^2);
    else
        int1 = (lambdaMm*sin(2*lambdaMm*x2))/4 + (lambdaMm^2*x2)/2 + (constm^2*lambdaPm*sinh(2*lambdaPm*x2))/4 + (constm^2*lambdaPm^2*x2)/2 + 2*constm*lambdaMm*sinh(lambdaPm*x2)*cos(lambdaMm) - ...
            (lambdaMm*sin(2*lambdaMm*x1))/4 + (lambdaMm^2*x1)/2 + (constm^2*lambdaPm*sinh(2*lambdaPm*x1))/4 + (constm^2*lambdaPm^2*x1)/2 + 2*constm*lambdaMm*sinh(lambdaPm*x1)*cos(lambdaMm);
 
        int2 =  (constm^2*lambdaPm^5*(sinh(2*lambdaPm*x2) + 2*lambdaPm*x2))/4 -(2*constm*lambdaMm^3*lambdaPm^3*(lambdaMm*cosh(lambdaPm*x2)*sin(lambdaMm*x2) + lambdaPm*cos(lambdaMm*x2)*sinh(lambdaPm*x2)))/(lambdaMm^2 + lambdaPm^2) + (constm^2*lambdaPm^5*(sinh(2*lambdaPm*x2) + 2*lambdaPm*x2))/4 - ...
            (constm^2*lambdaPm^5*(sinh(2*lambdaPm*x1) + 2*lambdaPm*x1))/4 -(2*constm*lambdaMm^3*lambdaPm^3*(lambdaMm*cosh(lambdaPm*x1)*sin(lambdaMm*x1) + lambdaPm*cos(lambdaMm*x1)*sinh(lambdaPm*x1)))/(lambdaMm^2 + lambdaPm^2) + (constm^2*lambdaPm^5*(sinh(2*lambdaPm*x1) + 2*lambdaPm*x1))/4;

end
    
    psiLn = ComputeModeAtL(lambdaMn, lambdaPn, L);
    psipLm = ComputeModeDiffAtL(lambdaMm, lambdaPm, L);
    psipppLm = ComputeModeDiffAtL(lambdaMm, lambdaPm, L);

    f = psiLn*(T0*psipLm - EI*psipppLm) - (T0*int1 + EI*int2);
end