...
MassMat2      = zeros(Nmodes,Nmodes) ;
StiffMat     = zeros(Nmodes,Nmodes) ;
gg           = (S * Wzc) ;
fInv         = gg * gg.' ;
wMass        = rAb / (Wzc.' * Wzc) ;
wStiff       = EIb / (Wzc.' * Wzc) / (Wzc.' * (fInv^(-1) * S) * Wzc );
for m = 1 : Nmodes
    for n = m : Nmodes
        lmm      = sqrt((sqrt(T0^2 + 4*EI*rA*omvec(m)^2) - T0)/2/EI) ;
        lpm      = sqrt((T0 + sqrt(T0^2 + 4*EI*rA*omvec(m)^2))/2/EI) ;
        lmn      = sqrt((sqrt(T0^2 + 4*EI*rA*omvec(n)^2) - T0)/2/EI) ;
        lpn      = sqrt((T0 + sqrt(T0^2 + 4*EI*rA*omvec(n)^2))/2/EI) ;
