...
%           Modal system - Explicit (1st order accurate)
%               Riccardo Russo
...
tic
for i = 1:timeSamples  
    eta = betaVecTR*(x - xPrev)/k - bowVel(i);
    
    xNext = (2 - k^2*eigenFreqs.^2).*x - xPrev - k^2*Fb(i)*alphaVec*sqrt(2*a)*eta*exp(-a*eta^2 + 0.5);
    ...
    if realTimeDraw
        plot(vNext,'LineWidth',lineWidth);
        ylabel("u(t,x_o) (m)");
        ylim([-2e-4,2e-4]);
        drawnow
    %     pause(0.05);
